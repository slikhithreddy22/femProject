...
type WorkoutEntry struct {
	ID              int      `json:"id"`
	ExcerciseName   string   `json:"exercise_name"`
	Sets            int      `json:"sets"`
	Reps            *int     `json:"reps"`
...
func NewPostgresWorkout(db *sql.DB) *PostgresWorkoutStore {
...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	`
		_, err = tx.Exec(query, workout.ID, entry.ExcerciseName, entry.Sets, entry.Reps, entry.DurationSeconds, entry.Weight, entry.Notes, entry.OrderIndex)
		if err != nil {
			return nil, err
		}
	}
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) (*Workout, error) {
	...
	defer tx.Rollback()
	query := `UPDATE workouts SET title = $1,
		description = $2,
		duration_minutes = $3,
		calories_burned = $4
		WHERE id = $5`
